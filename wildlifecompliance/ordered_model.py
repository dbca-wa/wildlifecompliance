...
class OrderedModelBase(models.Model):
    ...
    def save(self, *args, **kwargs):
        if getattr(self, self.order_field_name) is None:
            c = self.get_ordering_queryset().aggregate(Max(self.order_field_name)
    ...
    def delete(self, *args, **kwargs):
        ...
        qs.filter(**{self.order_field_name + '__gt': getattr(self,
                                                             self.order_field_name)}) .update(**update_kwargs)
    ...
    def _move(self, up, qs=None):
        ...
        if up:
            qs = qs.order_by('-' +
                             self.order_field_name) .filter(**{self.order_field_name +
                                                               '__lt': getattr(self, self.order_field_name)})
        else:
            qs = qs.filter(**{self.order_field_name +
        ...
        try:
            ...
            # already first/last
            return
        order, replacement_order = getattr(
            self, self.order_field_name), getattr(
            ...
            raise ValueError(
                "{0!r} can only be swapped with instances of {1!r} with equal {2!s} fields.".format(
                    ...
                    ' and '.join(
                        [
                            "'{}'".format(
                                o[0]) for o in self._get_order_with_respect_to()])))
        order, replacement_order = getattr(
            self, self.order_field_name), getattr(
            replacement, self.order_field_name)
        setattr(self, self.order_field_name, replacement_order)
    ...
    def above(self, ref, extra_update=None):
        ...
        if getattr(
            ...
            o = self.get_ordering_queryset() .filter(
                **{
                    self.order_field_name +
                    '__lt': getattr(
                        ref,
                        self.order_field_name)}) .aggregate(
                Max(
                    self.order_field_name)) .get(
                        self.order_field_name +
                '__max') or 0
    ...
    def top(self, extra_update=None):
        ...
        """
        o = self.get_ordering_queryset().aggregate(Min(self.order_field_name)
    ...
    def bottom(self, extra_update=None):
        ...
        """
        o = self.get_ordering_queryset().aggregate(Max(self.order_field_name)
                                                   ).get(self.order_field_name + '__max')
        self.to(o, extra_update=extra_update)
